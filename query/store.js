    ...
    return {
        init: async () => {
            ...
            for (let i = 0; i < songList.length; ++i) {
                ...
                let v = [genre, songName, artistName, mp3File, imageArt, positivity, energy, rhythm, liveliness];

                currentInsert = "INSERT INTO songs" +
        ...
        /** CREATE Operations */
        createSong: async (genre, songName, artistName, mp3File, imageArt, positivity, energy, rhythm, liveliness) => {
            // Check to see if a record with the same songName and artistName is already
            // in the database
            let queryTextOne = 'select * from songs where song_name = $1 and artist_name = $2';
        
            let songInDatabase = await client.query(queryTextOne, [songName, artistName]);
            let songInDatabaseRows = songInDatabase.rows;

            if (songInDatabaseRows.length === 1) {
                return [ { msg: 'A record with the same songName and artistName is already in the database' } ];
            }
        ...
        readSongById: async (id) => {
            let songById = await client.query('select * from songs where id = $1;', [id]);
            return songById.rows;
        },

        readSong: async (songName, artistName) => {
            let queryText = 'select * from songs where song_name = $1 and artist_name = $2';
            let foundSong = await client.query(queryText, [songName, artistName]);
            return foundSong.rows;
        },

        readSongsByCharacteristics: async (positivity, energy, rhythm, liveliness) => {
            let queryText = 'select * from songs where positivity > $1 and ' +
            'energy > $2 and rhythm > $3 and liveliness > $4;';
            let foundSongs = await client.query(queryText, [positivity, energy, rhythm, liveliness]);
            return foundSongs.rows;
        },

        readSongsByCharacteristicsAndGenre: async (genre, positivity, energy, rhythm, liveliness) => {
            let queryText = 'select * from songs where genre = $1 and positivity > $2 and ' +
            'energy > $3 and rhythm > $4 and liveliness > $5;';

            let foundSongs = await client.query(queryText, [genre, positivity, energy, rhythm, liveliness]);
            return foundSongs.rows;
        },

        readSongsByMode: async (positivity, energy, rhythm, liveliness, positivitySign, energySign, rhythmSign, livelinessSign) => {
            let queryText = `select * from songs where positivity ${positivitySign} $1 and ` +
        ...
        /** UPDATE Operations */
        updateSong: async (genre, songName, artistName, mp3File, imageArt, positivity, energy, rhythm, liveliness) => {
            ...
            let updatedArray = [genre, mp3File, imageArt, positivity, energy, rhythm, liveliness, songName, artistName];
            let res = await client.query(queryText, updatedArray);
            return res.rows;
        },

        deleteSong: async (songName, artistName) => {
            let res = await client.query('delete from songs where song_name = $1 and artist_name = $2 returning *', [songName, artistName]);

...
app.post('/events', async (req, res) => {
    ...
    if (event === undefined || typeof event !== "object") {
        res.status(400).send({ msg: 'The event be an object, have an event-type, and have event data'});
        logger.http(JSON.stringify({ message: "response", method: "POST", endpoint: "events", statusCode: res.statusCode }));
        logger.error('EventType Error: Events must be an object with a type and data object');
        return;
    }
    ...
    if (type === "SongCreated") {
        for (const port of servicePorts) {
            try {
                console.log(`(${process.pid}) Event Bus (Sending Event to ${port}) ${type}`);
                ...
                const msgOne = 'A record with the same songName and artistName is already in the database';
                if (sentEventJson.hasOwnProperty('msg') && sentEventJson['msg'] === msgOne) {
                ...
                if (sentEventJson.hasOwnProperty('msg') && sentEventJson['msg'] === msgTwo) {
                ...
                if (sentEventJson === undefined) {
                    res.status(500).send({ msg: `The Event Bus did not receive a response from port ${port}`});
                    logger.error(JSON.stringify({ msg: `The Event Bus did not receive a response from port ${port}` }));
                    logger.http(JSON.stringify({ message: "response", method: "POST", endpoint: "events", statusCode: res.statusCode }));
                    return;
                }
    ...
    if (type === "ModeCreated") {
        console.log('hello');
        try {
            console.log(`(${process.pid}) Event Bus (Sending Event to ${modePort}) ${type}`);
            ...
            if (sentEventJson === undefined) {
                ...
                res.status(500).send({ msg: 'The modes service could not handle the request right now'});
                return;
            }
    ...
    if (type === "SongUpdated") {
        for (const port of servicePorts) {
            try {
                ...
                if (sentEventJson.hasOwnProperty('msg') && sentEventJson['msg'] === msgOne) {
                    res.status(404).send({ msg: `The song with ID ${event.data['id']} was not found in the database`});
                    logger.error(JSON.stringify({ msg: msgOne }));
                    logger.http(JSON.stringify({ message: "response", method: "PUT", endpoint: "events", statusCode: res.statusCode }));
                    return;
                }
                ...
                if (sentEventJson.hasOwnProperty('updatedSong') && 
                    sentEventJson['updatedSong']['songName'] === msgTwo['updatedSong']['songName'] &&
                    sentEventJson['updatedSong']['artistName'] === msgTwo['updatedSong']['artistName']) {
                        res.status(201).send({ updatedSong });
                        logger.info(JSON.stringify({ msg: `Successfully updated song ${sentSong['songName']} by ${sentSong['artistName']}` }));
                        logger.http(JSON.stringify({ message: "response", method: "PUT", endpoint: "events", statusCode: res.statusCode }));
                        return;
                }
        ...
        for (const port of servicePorts) {
            try {
                ...
                if (sentEventJson.hasOwnProperty('msg') && sentEventJson['msg'] === msgTwo) {
                    res.status(404).send({ msg: `The song with ID ${event.data['id']} was not found in the database`});
                    logger.http(JSON.stringify({ message: "response", method: "DELETE", endpoint: "events", statusCode: res.statusCode }));
                    logger.error(JSON.stringify({ msg: `The song with ID ${event.data['id']} was not found in the database` }));
                    return;
                }
                ...
                if (sentEventJson.hasOwnProperty('deletedSong') && 
                    sentEventJson['deletedSong']['songName'] === event.data['songName'] &&
                    sentEventJson['deletedSong']['artistName'] === event.data['artistName']) {
                        res.status(201).send({ deletedSong });
                        logger.http(JSON.stringify({ message: "response", method: "DELETE", endpoint: "events", statusCode: res.statusCode }));
                        logger.error(JSON.stringify({ msg: `The song ${deletedSong['songName']} by artist ${deletedSong['artistName']} was successfully deleted` }));
                        return;
                }
